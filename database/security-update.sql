-- =====================================================
-- ATUALIZAÇÃO DE SEGURANÇA - SISTEMA DE CONVITES
-- =====================================================
...
-- 5. Criar políticas RLS para client_invites
CREATE POLICY IF NOT EXISTS professionals_can_manage_own_invites ON client_invites
    FOR ALL USING (professional_id IN (
        SELECT id FROM professionals WHERE email = auth.jwt() ->> 'email'
    ));

CREATE POLICY IF NOT EXISTS public_can_read_valid_invites ON client_invites
    FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS public_can_manage_sessions ON client_sessions
    FOR ALL USING (true);
...
-- INSERT INTO client_invites (professional_id, client_email, client_name, invite_code, expires_at)
-- SELECT 
--     p.id,
--     'cliente@exemplo.com',
--     'Cliente Exemplo',
...
-- 15. Mostrar estrutura das novas tabelas
\d client_invites;
\d client_sessions;
